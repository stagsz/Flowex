...
**Last Updated:** 2026-01-25 (Add remember last accessed folder for cloud storage - IMP-07)
**Status:** In Progress
...
- GDPR: Added cookie consent management (Article 7 - Conditions for consent) - `9215288`
  - CookieConsentBanner component with customizable preferences dialog
  - Four cookie categories: necessary (always on), analytics, functional, marketing
  - LocalStorage persistence with version tracking for policy change re-prompts
  - Full accessibility support (ARIA roles, screen reader compatible)
  - 39 comprehensive tests for utilities and component behavior
...
- UX: Added 'E' keyboard shortcut to edit selected item (VAL-09) - `6809a50`
  - Press 'E' to focus and select the tag input field for quick editing
  - Added tagInputRef for programmatic focus management
  - Updated keyboard shortcuts help dialog with new shortcut
  - Completes missing keyboard shortcut from validation-interface.md spec
- Projects: Added edit project functionality (PM-02) - `b1e8b88`
  - Edit option in project dropdown menu with pencil icon
  - Edit Project dialog with name and description fields
  - Wired up PATCH /api/v1/projects/{id} API call
  - Optimistic UI update on successful save
  - Error handling with user-friendly messages
- Projects: Added drawing_count to project API responses (DB-01) - `5a8e3a6`
  - Backend: Added drawing_count field to ProjectResponse model
  - Backend: Efficient GROUP BY query for all projects in list view
  - Backend: Individual count queries for get/update endpoints
  - Frontend: Updated ProjectsPage to display actual drawing counts
  - Tests: 9 new project model and drawing_count tests
- UX: Added component list sorting functionality (VAL-06) - `decfea5`
  - Sort dropdown with 4 options: Tag, Type, Confidence, Status
  - Toggle button to switch ascending/descending order
  - Status sorting: flagged → pending → verified
  - Applied after filtering for correct behavior
  - Completes the sortable requirement from validation-interface.md spec
- Dashboard: Added drawing list sorting functionality (DB-06) - `41eefdb`
  - Sort dropdown with 3 options: Name, Date, Status
  - Toggle button to switch ascending/descending order
  - Default sort by date (descending) to show newest drawings first
  - Status sorting follows workflow: uploaded → processing → review → complete → error
  - Completes the sortable requirement from project-dashboard.md spec
- Admin: Added Audit Log Viewer page (SEC-04 UI) - `c77c620`
  - AuditLogsPage.tsx with paginated table view (25 entries per page)
  - Filters: action type dropdown, date range pickers, user search
  - Color-coded action categories (Auth, Users, Projects, Drawings, etc.)
  - Export to CSV functionality for compliance reports
  - Responsive design for mobile/desktop layouts
  - Admin-only route at /admin/audit-logs with sidebar navigation
  - Completes SEC-04 by providing UI for backend audit log endpoint
- Projects: Added archive and delete confirmation dialog (PM-03) - `b83c688`
  - Delete confirmation dialog with warning about project drawings
  - Archive option in project dropdown menu for non-destructive removal
  - Shows drawing count in confirmation to inform user of impact
  - Loading state during delete to prevent double-clicks
  - Suggests archiving as alternative to permanent deletion
  - Completes PM-03 from project-dashboard.md spec
- Admin: Added Organization Members management page (UM-01 to UM-04 UI) - `0abd237`
  - OrganizationMembersPage.tsx with complete team management UI
  - Members list with role badges (admin/member/viewer) and join dates
  - Invite user dialog with email input and role selection
  - Pending invitations list with copy link and revoke actions
  - Change role dialog for updating member permissions
  - Remove member dialog with confirmation and warnings
  - Admin-only route at /admin/members with sidebar navigation
  - Completes frontend UI for backend organization management API
- Export: Added batch export feature for multiple drawings (DB-07)
  - Backend: POST /api/v1/exports/batch endpoint for batch export requests
  - Backend: GET /api/v1/exports/batch/{job_id}/status endpoint for progress tracking
  - Backend: Background task creates ZIP file containing all exported files
  - Backend: Supports DXF, data lists, and checklist export types in batch
  - Backend: Handles partial failures gracefully (continues with remaining drawings)
  - Frontend: Multi-select checkboxes on DrawingsPage for exportable drawings
  - Frontend: "Select All" checkbox header for bulk selection
  - Frontend: Bulk action bar with Export All button (shown when items selected)
  - Frontend: BatchExportDialog component with export options and progress tracking
  - 51 export tests passing, all type checks and lint clean
  - Completes DB-07 from project-dashboard.md spec
- Dashboard: Added Usage & Billing card with real API data (UB-01 to UB-03) - `ad8ed0d`
  - UsageStatsCard component showing plan usage, limits, and upgrade button
  - Fetches organization usage from GET /api/v1/organizations/{id}/usage endpoint
  - Progress bar with color-coded warnings (yellow at 80%, red at 100%)
  - Displays plan name, usage count/limit, remaining P&IDs, and reset date
  - DashboardPage now fetches real project/drawing counts from API
  - Loading states with spinner for better UX
  - 9 comprehensive tests for UsageStatsCard component
  - Completes UB-01 (plan details), UB-02 (P&IDs used), UB-03 (remaining)
- Dashboard: Added progress percentage to drawings API and UI (DB-02) - `4046910`, `cf0ac37`
  - Backend: Added `progress_percentage` field to DrawingResponse model
  - Backend: `calculate_progress_percentage()` function computes progress from status and verification
  - Progress mapping: uploaded=0%, processing=50%, review=80-100% (based on verification), complete=100%
  - Efficient batch query for verification stats when listing drawings
  - Frontend: DrawingsPage uses real `progress_percentage` from API
  - Frontend: Shows progress bar with percentage for processing and review statuses
  - 9 comprehensive tests for progress calculation (81 total drawing tests)
  - Completes DB-02 from project-dashboard.md spec
- Dashboard: Added project activity feed endpoint (DB-09) - `ecdc4e0`
  - Backend: GET /api/v1/projects/{id}/activity endpoint
  - Shows recent user activities: uploads, validations, exports, symbol/line operations
  - Leverages existing AuditLog infrastructure (SEC-04)
  - User-friendly action display names per spec (e.g., "uploaded", "completed_validation")
  - Limit query parameter (default 10, max 100)
  - Entity name resolution for drawings
  - 13 new tests for models and action mapping (22 total project tests)
- Dashboard: Added project activity feed UI to Dashboard (DB-09 UI) - `ebc042a`
  - ProjectActivityFeed component displays recent project activity
  - DashboardActivityFeed wrapper for organization-wide activity (most recent project)
  - Integrated into DashboardPage, replacing mock Recent Drawings section
  - Shows user actions with icons: uploads, validations, exports, symbol/line operations
  - Displays relative timestamps (e.g., "2h ago", "15m ago")
  - Includes loading, error, and empty states with retry functionality
  - Links to audit logs for full activity view
  - 15 comprehensive tests for component behavior
  - Completes DB-09 from project-dashboard.md spec (full stack: backend + frontend)
- Users: Added My Activity page (UM-05)
  - Backend: GET /api/v1/users/me/activity endpoint returns user's own activity history
  - Backend: Paginated results with filters (action type, date range)
  - Frontend: MyActivityPage component with activity table and filters
  - Frontend: Export to CSV functionality for personal records
  - Navigation: "My Activity" link added to user dropdown menu in Header
  - Routing: /my-activity route added to App.tsx
  - 8 new tests for user activity endpoint and response models
  - Completes UM-05 from project-dashboard.md spec
- Projects: Added project-level member permissions (PM-05) - `392a47d`
  - Backend: ProjectMember model with ProjectRole enum (owner/editor/viewer)
  - Backend: Alembic migration 007 for project_members table
  - Backend: API endpoints for project member management (list, add, update role, remove)
  - Backend: Add by user ID or email address
  - Backend: GET /projects/{id}/members/me for permission checking
  - Backend: Project creators automatically added as owners
  - Backend: Safety checks prevent removing/demoting last owner
  - 19 new tests for project member models and functionality
  - Enables per-project permissions independent of organization roles
  - Completes PM-05 backend from project-dashboard.md spec
- Projects: Added project members management UI (PM-05 UI) - `0e7b7f6`
  - Frontend: ProjectMembersDialog.tsx component for managing project members
  - Frontend: Displays member list with role badges (owner/editor/viewer)
  - Frontend: Add member by email with role selection dialog
  - Frontend: Change role and remove member dialogs (owners only)
  - Frontend: Permission-based controls (only owners can modify)
  - Frontend: Loading, error, and empty states with retry functionality
  - Frontend: Integrated into ProjectsPage dropdown menu as "Members" option
  - Completes PM-05 frontend from project-dashboard.md spec (full stack complete)
- Billing: Added billing and plans page (UB-04) - `165abff`
  - Frontend: SettingsBillingPage.tsx with complete billing UI
  - Current plan overview with usage stats and progress bar
  - Plan comparison table with feature matrix (Free, Starter, Pro, Enterprise)
  - Pricing display with upgrade CTAs and contact sales flow
  - FAQ section for common billing questions
  - Fixes broken /settings/billing route that caused 404 errors
  - Completes UB-04 from project-dashboard.md spec
- GDPR: Added breach notification system (GDPR-07) - `4b1ac0f`
  - SecurityBreach model with severity (low/medium/high/critical), status, and category
  - Alembic migration 008 for security_breaches table with indexes
  - Complete CRUD API endpoints at /api/v1/breaches (admin only)
  - 72-hour notification deadline tracking per GDPR Article 33
  - POST /breaches/{id}/notify-authority - record authority notification
  - POST /breaches/{id}/notify-users - record user notification (Article 34)
  - GET /breaches/stats - dashboard statistics with deadline monitoring
  - Audit logging for all breach operations (5 new AuditAction types)
  - 23 comprehensive tests for models, endpoints, and deadline calculation
  - Completes GDPR-07 from authentication-security.md spec
- Cloud: Added remember last accessed folder per connection (IMP-07) - `5beba1e`
  - lastAccessedFolders state in cloudStore with localStorage persistence (zustand persist)
  - Saves current folder to lastAccessedFolders on each browse action
  - CloudFilePicker and CloudFolderPicker start at last accessed folder when reopened
  - Improves UX by remembering user's browsing position per cloud connection
  - Completes IMP-07 from cloud-storage-integration.md spec
...
- [x] Implement cookie consent management (GDPR-06) - `9215288`
  - CookieConsentBanner component with customizable preferences dialog
  - Four cookie categories: necessary (required), analytics, functional, marketing
  - LocalStorage persistence with version tracking for policy change re-prompts
...
- [x] Implement breach notification system (GDPR-07) - `4b1ac0f`
  - SecurityBreach model with severity, status, and breach category
  - Alembic migration 008 for security_breaches table
  - CRUD API endpoints at /api/v1/breaches (admin only)
  - 72-hour notification deadline tracking per GDPR Article 33
  - Authority and user notification recording (Article 34)
  - Dashboard statistics endpoint with deadline monitoring
  - 5 new AuditAction types for breach audit trail
  - 23 tests for models, endpoints, and deadline calculation
...
- [x] **Fix mypy type errors in batch export and Celery tasks** - `702f778`
  - Fixed variable type confusion in exports.py by using distinct service variable names
    (dxf_service, list_service, checklist_service) for different service types across branches
  - Updated Celery task decorator type ignores from `[misc]` to `[untyped-decorator]`
    in cloud.py and processing.py (mypy error code changed again)
  - All 345 backend tests pass, mypy reports 0 errors (56 source files)
